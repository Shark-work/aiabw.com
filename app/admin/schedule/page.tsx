...
type ScheduleRow = { day_of_week: number; topic_id: string | null; topic_name?: string | null };
...
export default function AdminSchedulePage() {
  ...
  async function save(dayOfWeek: number, topicId: string | null) {
    const res = await fetch("/api/admin/schedule", { method: "PUT", headers: { "Content-Type": "application/json", "x-admin-email": email }, body: JSON.stringify({ day_of_week: dayOfWeek, topic_id: topicId }) });
  ...
  return (
    <AdminShell email={email} active="schedule">
      <div className="mb-6"><h2 className="text-2xl font-semibold">排期管理</h2><p className="mt-1 text-sm text-white/60">周一至周日从活跃题材中选择对应题材并自动保存。</p></div>
